...
Also found, NOT fixed (flagged for the user, needs a judgment call, not a guess): this repo's full git history (177 commits) carries the real name/email `alextria`/`grumpybollocks <coteanualex@gmail.com>` and one commit as `Andrei Colta <tec36486@adobe.com>` -- same class of exposure the peer G910 session scrubbed via `git-filter-repo` for its own repo, never done here. `packaging/g510-lcd/PKGBUILD`'s Maintainer line also has the same real email in current file content, not just history. The user was told directly and said leave it as-is for now.
